...
class AddPage(DataMixin, View):
    ...
    def get(self, request):
        ...
        data = {"title": f"Добавление статьи", "menu": menu, "form": form}
...
class ShowPost(DataMixin, DetailView, Qawsda):
    ...
    def get_context_data(self, **kwargs):
        ...
        return self.get_mixin_context(
            context, title=context["post"].title, comment=comment, form=AddCommentForm
        )
...
class Search(DataMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super().get_context_data(**kwargs)
        return self.get_mixin_context(
            context, title="Поиск", s=f"s={self.request.GET.get('s')}&"
        )
